            ...
            override fun onResponse(call: Call<ContactResponse>, response: Response<ContactResponse>) {
                if (response.isSuccessful) {
                    response.body()?.let {
                        ...
                        context?.let { ctx ->
                            Toast.makeText(ctx, "Response was successful but body is null", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    context?.let { ctx ->
                        Toast.makeText(ctx, "Failed to send message: ${response.message()}", Toast.LENGTH_SHORT).show()
                    }
                }
            }
